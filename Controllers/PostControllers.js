...
module.exports = {
  ...
  getLikedPost: async (req, res) => {
    try {
      ...
      console.log(sql1, "trigger getlike");
      const sql1Result = await query(sql1, [id]);
      console.log(sql1Result, "sql1");
      let sql2 = `SELECT * FROM posts WHERE id = ?;`;
      for (let i = 0; i < sql1Result.length; i++) {
        console.log(sql1Result[i].postId, "sql21");
  ...
  addPost: (req, res) => {
    try {
      ...
      upload(req, res, (err) => {
        ...
        data.userid = id;
        console.log(imagePath, "path check");
        //your query has a problem thats why i comment it
        var sql = `INSERT INTO posts (image, userid, caption) VALUES (${db.escape(
          data.image
        )}, ${data.userid}, ${db.escape(data.caption)})`; //hmmm lemme see your db
        db.query(sql, (err, results) => {
          ...
          //inter into [table_name] SET [what column you wanna set]
          //thing is this is for UPDATE not insert
          //yes
          //well lets do this
          // if(data.caption === ''){
          ...
          console.log("ini result upload", results);
          ...
          db.query(sql, id, (err, results) => {
            if (err) {
              return res
                .status(500)
                .json({ message: "Server Error2", error: err.message });
            }
      ...
      return res
        .status(500)
        .json({ message: "Server Error3", error: err.message });
    }
  },
          ...
          try {
            ...
            db.query(sql, data, (err1, results1) => {
              if (err1) {
                ...
                return res
                  .status(500)
                  .json({ message: "Server Error", error: err1.message });
              }
              if (imagePath) {
