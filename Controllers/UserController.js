...
module.exports = {
  register: async (req, res) => {
    try {
      ...
      if (data.password.length > 50)
        throw { message: "Password Maximum 50 Character" };
      ...
      const findUsername = await query(query01, data.username).catch(
        (error) => {
          throw error;
        }
      );
        ...
        try {
          ...
          db.query(query3, [token, insertUser.insertId], (err1, result1) => {
            try {
              ...
              fs.readFile(
                "C:/Users/Jackfrost/Documents/JWCDAH/Social-Media-5/backend-sosmed-main/Public/Template/index.html",
                {
                  encoding: "utf-8",
                },
                (err, file) => {
                  ...
                  const newTemplateResult = newTemplate({
                    bebas: data.email,
  ...
  confirmation: (req, res) => {
    ...
    db.query(
      "SELECT * FROM users WHERE id = ? AND is_confirmed = 0",
      id,
      (err, result) => {
        try {
          if (err) throw err;
            ...
            try {
              ...
              db.query(query3, [token, id], (err1, result1) => {
                try {
                  ...
                  fs.readFile(
                    "C:/Users/Jackfrost/Documents/JWCDAH/Social-Media-5/backend-sosmed-main/Public/Template/index2.html",
                    {
                      encoding: "utf-8",
                    },
                    (err, file) => {
        ...
        upload(req, res, (err) => {
          if (err) {
            return res.status(500).json({
              message: "Upload post profile picture failed !",
              error: err.message,
            });
          }
          ...
          try {
            ...
            db.query(sql, data, (err1, results1) => {
              ...
              // if (imagePath) {
              //   fs.unlinkSync("./Public" + results[0].profileimage);
              // }
